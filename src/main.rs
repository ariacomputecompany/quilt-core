...
use sync::{ContainerState, MountType, SyncEngine};
...
use quilt::quilt_runtime_server::{QuiltRuntime, QuiltRuntimeServer};
use quilt::quilt_service_server::{QuiltService, QuiltServiceServer};
use quilt::{
    ContainerEvent as ProtoContainerEvent, ContainerMetric, ContainerStatus,
    CreateContainerRequest, CreateContainerResponse, CreateVolumeRequest, CreateVolumeResponse,
    ExecContainerRequest, ExecContainerResponse, GetContainerByNameRequest,
    GetContainerByNameResponse, GetContainerLogsRequest, GetContainerLogsResponse,
    GetContainerStatusRequest, GetContainerStatusResponse, GetHealthRequest, GetHealthResponse,
    GetMetricsRequest, GetMetricsResponse, GetSystemInfoRequest, GetSystemInfoResponse,
    HealthCheck, InspectVolumeRequest, InspectVolumeResponse, KillContainerRequest,
    KillContainerResponse, ListContainersRequest, ListContainersResponse, ListVolumesRequest,
    ListVolumesResponse, RemoveContainerRequest, RemoveContainerResponse, RemoveVolumeRequest,
    RemoveVolumeResponse, StartContainerRequest, StartContainerResponse, StopContainerRequest,
    StopContainerResponse, StreamEventsRequest, SystemMetrics as ProtoSystemMetrics,
};
...
impl QuiltServiceImpl {
    pub async fn new() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let sync_engine = Arc::new(
            SyncEngine::new_with_network_config(
                "quilt.db",
    ...
    async fn has_active_container_state(&self) -> Result<bool, Status> {
        let statuses = self
            .sync_engine
        ...
        Ok(statuses
            .iter()
            .any(|s| s.state == ContainerState::Running || s.state == ContainerState::Pending))
    }
}
...
impl QuiltService for QuiltServiceImpl {
    async fn create_container(
        ...
        let config = sync::containers::ContainerConfig {
            ...
            command: if req.command.is_empty() {
                if req.async_mode {
                    ...
                    return Err(Status::invalid_argument(
                        "Command required for non-async containers",
                    ));
                }
            } else {
                req.command.join(" ")
            ...
            memory_limit_mb: if req.memory_limit_mb > 0 {
                Some(req.memory_limit_mb as i64)
            } else {
                None
            },
            cpu_limit_percent: if req.cpu_limit_percent > 0.0 {
                Some(req.cpu_limit_percent as f64)
            } else {
                None
            },
        ...
        match self.sync_engine.create_container(config).await {
            Ok(_network_config) => {
                ...
                ConsoleLogger::success(&format!(
                    "Container {} created with network config",
                    container_id
                ));
                ...
                let _ = self
                    .sync_engine
                    .store_container_log(&container_id, "info", "Container created and configured")
                    .await;
                ...
                for mount in req.mounts {
                    ...
                    match InputValidator::parse_volume(&mount_string) {
                        Ok(parsed_mount) => {
                            ConsoleLogger::debug(&format!(
                                "Mount validation passed for {}: {} -> {} (readonly: {})",
                                container_id,
                                parsed_mount.source,
                                parsed_mount.target,
                            ...
                            if parsed_mount.readonly != mount.readonly {
                                ConsoleLogger::debug(&format!(
                                    "Mount readonly flag updated from {} to {} for {}",
                        ...
                        Err(e) => {
                            ConsoleLogger::warning(&format!(
                                "Mount parsing validation failed for {}: {}",
                                container_id, e
                            ));
                    ...
                    ConsoleLogger::debug(&format!(
                        "Mount security validation passed for {}: {} -> {}",
                    ...
                    {
                        ConsoleLogger::error(&format!(
                            "Failed to add mount for container {}: {}",
                            container_id, e
                        ));
                    ...
                    ConsoleLogger::success(&format!(
                        "Mount successfully added for {}: {} -> {} (readonly: {})",
                ...
                tokio::spawn(async move {
                    ...
                    ConsoleLogger::info(&format!(
                        "⏰ [TASK-SPAWN] Starting container {} with {:?} timeout",
                    ...
                    match startup_result {
                        ...
                        Ok(Err(e)) => {
                            ...
                            let _ = sync_engine
                                .update_container_state(&container_id_clone, ContainerState::Error)
                                .await;
                        }
                        Err(_) => {
    ...
    async fn get_container_status(
        ...
        let container_id = if !req.container_name.is_empty() {
            match self
                .sync_engine
                .get_container_by_name(&req.container_name)
                .await
            {
                Ok(id) => id,
                Err(_) => {
        ...
        match self.sync_engine.get_container_status(&container_id).await {
            Ok(status) => {
                ...
                Ok(Response::new(GetContainerStatusResponse {
                    ...
                    error_message: if status.state == ContainerState::Error {
    ...
    async fn get_container_logs(
            ...
            .await
        {
            ...
            Err(e) => {
                tracing::warn!(
                    "Failed to get sync engine logs for container {}: {}",
                    container_id,
                    e
                );
            }
            ...
            if let Some(rootfs) = status.rootfs_path {
                if FileSystemUtils::exists(&rootfs) {
                    if FileSystemUtils::is_directory(&rootfs) {
                        ...
                        let important_files =
                            vec!["/bin/sh", "/bin/busybox", "/usr/local/bin/quilt_ready"];
                            ...
                            if FileSystemUtils::exists(&full_path) {
                                if FileSystemUtils::is_file(&full_path) {
                                    ...
                                    all_logs.push(quilt::LogEntry {
                                        timestamp: current_time + 2,
                                        message: format!(
                                            "[INSPECT] {} - broken symlink detected",
                                            file
                                        ),
                                    });
                                }
    ...
    async fn stop_container(
        ...
        match runtime.stop_container(&container_id) {
            Ok(()) => {
                ...
                if let Err(e) = self
                    .sync_engine
                    .update_container_state(&container_id, ContainerState::Exited)
                    .await
                {
                    ConsoleLogger::warning(&format!(
                        "Failed to update container state in sync engine: {}",
                        e
                    ));
                }
                ...
                let _ = self
                    .sync_engine
                    .store_container_log(&container_id, "info", "Container stopped successfully")
                    .await;
    ...
    async fn remove_container(
        ...
        match self.sync_engine.delete_container(&container_id).await {
            Ok(()) => {
                ...
                {
                    ConsoleLogger::warning(&format!(
                        "Failed to remove mounts for {}: {}",
                        container_id, e
                    ));
                }

                // Cleanup container logs (keep last 10 for debugging)
                if let Ok(cleaned_count) = self
                    .sync_engine
                    .cleanup_container_logs(&container_id, 10)
                    .await
                {
                    ConsoleLogger::debug(&format!(
                        "Cleaned up {} log entries for {}",
                        cleaned_count, container_id
                    ));
                }
                ...
                let container_pid = runtime
                    .get_container_info(&container_id)
                    .and_then(|info| info.pid);

                if let Err(e) = resource_manager
                    .cleanup_container_with_correlation(&container_id, container_pid)
                {
                    ConsoleLogger::warning(&format!(
                        "Resource correlation cleanup issues for {}: {}",
                        container_id, e
                    ));
                } else {
                    ConsoleLogger::debug(&format!(
                        "✅ Resource correlation cleanup completed for {}",
                        container_id
                    ));
                }
                ...
                if let Err(e) = runtime_result {
                    ConsoleLogger::warning(&format!(
                        "Runtime cleanup issues for {}: {}",
                        container_id, e
                    ));
                }

                ConsoleLogger::success(&format!(
                    "Container {} removed with comprehensive cleanup",
                    container_id
                ));
                ...
                let _ = self
                    .sync_engine
                    .store_container_log(&container_id, "info", "Container removed successfully")
                    .await;
    ...
    async fn exec_container(
        ...
        ConsoleLogger::debug(&format!(
            "🔍 [GRPC] Exec request for: {} with command: {:?}",
            container_id, req.command
        ));
        ...
        match self.sync_engine.get_container_status(&container_id).await {
            Ok(status) => {
                ...
                let escaped_command = command_to_execute
                    .replace("\\", "\\\\")
                    .replace("\"", "\\\"")
                    .replace("$", "\\$")
                    .replace("`", "\\`");
                ...
                let exec_cmd = if req.capture_output {
                    format!(
                        "nsenter -t {} -p -m -n -u -- chroot {} /bin/sh -c \"{}{}\"",
                        pid, rootfs_path, path_prefix, escaped_command
                    )
                } else {
                ...
                match CommandExecutor::execute_shell(&exec_cmd) {
                    Ok(result) => {
                        ConsoleLogger::debug(&format!(
                            "✅ [GRPC] Exec completed with exit code: {}",
                    ...
                    Err(e) => {
                        ConsoleLogger::warning(&format!(
                            "⚠️ [GRPC] CommandExecutor failed, trying runtime exec: {}",
                            e
                        ));
                        ...
                        match runtime.exec_container(
                            ...
                            Ok((exit_code, stdout, stderr)) => {
                                ConsoleLogger::debug(&format!(
                                    "✅ [GRPC] Runtime exec completed with exit code: {}",
                                    exit_code
                                ));

                                // Clean up temporary script on success
                                if req.copy_script
                                    && command_to_execute.starts_with("/tmp/quilt_exec_")
                                {
                                    let cleanup_cmd = format!(
                                        "nsenter -t {} -p -m -n -u -- chroot {} rm -f {}",
                                        pid, rootfs_path, command_to_execute
    ...
    async fn start_container(
        ...
        tokio::spawn(async move {
            if let Err(e) =
                start_container_process(&sync_engine, &container_id_clone, network_manager).await
            {
                ConsoleLogger::error(&format!(
                    ...
                    match ProcessUtils::send_signal(nix_pid, Signal::SIGKILL) {
                        Ok(()) => {
                            ...
                            if !ProcessUtils::is_process_running(nix_pid) {
                                ...
                                ConsoleLogger::warning(&format!(
                                    "Process {} still exists after SIGKILL",
                                    pid
                                ));
                            }
                            ...
                            let _ = self
                                .sync_engine
                                .set_container_exit_code(&container_id, -9)
                                .await;
    ...
    async fn create_volume(
        ...
        match self
            ...
            .await
        {
            Ok(volume) => Ok(Response::new(CreateVolumeResponse {
                success: true,
                error_message: String::new(),
            ...
            Err(e) => Ok(Response::new(CreateVolumeResponse {
                success: false,
                error_message: e.to_string(),
                volume: None,
            })),
    ...
    async fn remove_volume(
        ...
        match self.sync_engine.remove_volume(&req.name, req.force).await {
            Ok(()) => Ok(Response::new(RemoveVolumeResponse {
                success: true,
                error_message: String::new(),
            })),
            Err(e) => Ok(Response::new(RemoveVolumeResponse {
                success: false,
                error_message: e.to_string(),
            })),
        }
    ...
    async fn list_volumes(
        ...
        match self
            .sync_engine
            .list_volumes(if req.filters.is_empty() {
                None
            } else {
                Some(req.filters)
            })
        ...
        {
            Ok(volumes) => {
                let proto_volumes: Vec<quilt::Volume> = volumes
                    .into_iter()
                    .map(|v| quilt::Volume {
                        name: v.name,
                        driver: v.driver,
    ...
    async fn inspect_volume(
        ...
        match self.sync_engine.get_volume(&req.name).await {
            Ok(Some(volume)) => Ok(Response::new(InspectVolumeResponse {
                found: true,
                volume: Some(quilt::Volume {
                    name: volume.name,
                    driver: volume.driver,
            ...
            Ok(None) => Ok(Response::new(InspectVolumeResponse {
                found: false,
                volume: None,
                error_message: format!("Volume '{}' not found", req.name),
            })),
            Err(e) => Ok(Response::new(InspectVolumeResponse {
                found: false,
                volume: None,
                error_message: e.to_string(),
            })),
    ...
    async fn get_health(
        ...
        let (containers_total, containers_running) =
            match self.sync_engine.get_container_counts().await {
    ...
    async fn get_metrics(
        ...
        if !req.container_id.is_empty() {
            if req.start_time > 0 && req.end_time > 0 {
                ...
                if let Ok(historical_metrics) = self
                    .sync_engine
                    .get_metrics_history(
                        &req.container_id,
                        req.start_time,
                    ...
                    .await
                {
                    ...
                    let use_cached = if let Ok(Some(latest_metrics)) =
                        self.sync_engine.get_latest_metrics(&req.container_id).await
                    {
                    ...
                    if !use_cached {
                        ...
                        use crate::daemon::runtime::ContainerRuntime;
                        let runtime = ContainerRuntime::new();
                        let _runtime_stats = runtime
                            .get_container_stats(&req.container_id)
                            .unwrap_or_default();

                        if let Ok(metrics) = collector.collect_container_metrics(
                            &req.container_id,
                            status.pid.map(|p| p as i32),
                        ) {
                            container_metrics.push(ContainerMetric {
                                container_id: metrics.container_id.clone(),
                                timestamp: metrics.timestamp,
            ...
            if let Ok(containers) = self
                .sync_engine
                .list_containers(Some(ContainerState::Running))
                .await
            {
                let collector = MetricsCollector::new();
                for container in containers {
                    if let Ok(metrics) = collector
                        .collect_container_metrics(&container.id, container.pid.map(|p| p as i32))
                    {
                        container_metrics.push(ContainerMetric {
                            container_id: metrics.container_id.clone(),
                            timestamp: metrics.timestamp,
    ...
    async fn get_system_info(
        ...
        features.insert(
            ...
            "pid,mount,uts,ipc,network".to_string(),
        );
        ...
        limits.insert(
            "max_memory_per_container".to_string(),
            "unlimited".to_string(),
        );
        limits.insert(
            "max_cpus_per_container".to_string(),
            "unlimited".to_string(),
        );
        ...
        if let Ok(engine_stats) = self.sync_engine.get_stats().await {
            stats_features.insert(
                ...
                engine_stats.total_containers.to_string(),
            );
            stats_features.insert(
                "running_containers".to_string(),
                engine_stats.running_containers.to_string(),
            );
            stats_features.insert(
                "active_networks".to_string(),
                engine_stats.active_networks.to_string(),
            );
            stats_features.insert(
                "active_monitors".to_string(),
                engine_stats.active_monitors.to_string(),
            );
                ...
                .await
            {
                ...
                for container in containers {
                    ...
                    let (container_info, stats_result) =
                        self.runtime.get_container_info_and_stats(&container.id);
                    ...
                    if let Ok(stats) = stats_result {
                        ...
                        if let Some(info) = container_info {
                            if let Some(pid) = info.pid {
                                ...
                                stats_features.insert(
                                    format!("container_{}_pid", container.id),
                                    pid_i32.to_string(),
                                );
                            }
                        }
                ...
                if containers_with_stats > 0 {
                    ...
                    stats_features.insert(
                        "containers_with_runtime_stats".to_string(),
                        containers_with_stats.to_string(),
                    );
                }
            }
        }
    ...
    async fn stream_events(
        ...
        let stream = IntervalStream::new(tokio::time::interval(Duration::from_millis(100)))
            .map(move |_| {
                ...
                let proto_events: Vec<ProtoContainerEvent> = events
                    .into_iter()
    ...
    type StreamEventsStream =
        std::pin::Pin<Box<dyn futures::Stream<Item = Result<ProtoContainerEvent, Status>> + Send>>;
    ...
    async fn get_cleanup_status(
        ...
        let req = request.into_inner();
        let container_filter = if req.container_id.is_empty() {
            None
        } else {
        ...
        match self
            .sync_engine
            .cleanup_service
            .get_cleanup_tasks(container_filter)
            .await
        {
            Ok(tasks) => {
                let proto_tasks = tasks
                    .into_iter()
                    .map(|t| quilt::CleanupTask {
                        task_id: t.id,
    ...
    async fn list_container_cleanup_tasks(
        ...
        match self
            .sync_engine
            .cleanup_service
            .list_container_cleanup_tasks(&req.container_id)
            .await
        {
            Ok(tasks) => {
                let proto_tasks = tasks
                ...
                let proto_allocations = allocations
                    ...
                    .map(|a| quilt::NetworkAllocation {
                        ...
                        status: match a.status {
                            crate::sync::network::NetworkStatus::Allocated => {
                                "allocated".to_string()
                            }
                            crate::sync::network::NetworkStatus::Active => "active".to_string(),
                            crate::sync::network::NetworkStatus::CleanupPending => {
    ...
    async fn force_cleanup(
        ...
        if !req.container_id.is_empty() {
            match self
                .sync_engine
                .cleanup_service
                .force_cleanup(&req.container_id)
                .await
            {
                ...
                Err(e) => {
                    return Ok(Response::new(quilt::ForceCleanupResponse {
                        success: false,
        ...
        match self.sync_engine.cleanup_old_metrics(7).await {
            Ok(cleaned_metrics) => {
                if cleaned_metrics > 0 {
                    cleanup_messages
                        .push(format!("Cleaned {} old metric records", cleaned_metrics));
                }
            }
    ...
    async fn get_container_network(
        ...
        match self
            .sync_engine
            .get_network_allocation(&req.container_id)
            .await
        {
            Ok(allocation) => {
                ...
                match self.runtime.get_container_network(&req.container_id) {
                    Some(network_config) => {
                        let proto_config = quilt::ContainerNetworkConfig {
                            ...
                            status: format!(
                                "Network configured for container {}",
                                req.container_id
                            ),
                        };
                    ...
                    None => Ok(Response::new(quilt::GetContainerNetworkResponse {
                        success: false,
                        error_message: format!(
                            "No network configuration found for container {}",
                            req.container_id
                        ),
                        network_config: None,
                    })),
    ...
    async fn set_container_network(
        ...
        if let Some(proto_config) = req.network_config {
            ...
            match self
                .runtime
                .set_container_network(&req.container_id, network_config)
            {
    ...
    async fn setup_container_network_post_start(
        ...
        match self
            .runtime
            .setup_container_network_post_start(&req.container_id, &self.network_manager)
        {
            Ok(_) => Ok(Response::new(
                quilt::SetupContainerNetworkPostStartResponse {
                    success: true,
                    error_message: String::new(),
                },
            )),
    ...
    async fn comprehensive_network_cleanup(
        ...
        match self.sync_engine.comprehensive_network_cleanup().await {
            Ok(cleaned_resources) => {
                Ok(Response::new(quilt::ComprehensiveNetworkCleanupResponse {
                    cleaned_resources,
                    success: true,
                    error_message: String::new(),
                }))
...
#[tonic::async_trait]
impl QuiltRuntime for QuiltServiceImpl {
    async fn configure_node_subnet(
        &self,
        request: Request<quilt::ConfigureNodeSubnetRequest>,
    ) -> Result<Response<quilt::ConfigureNodeSubnetResponse>, Status> {
        let req = request.into_inner();
        let subnet = req.subnet.trim().to_string();
        if subnet.is_empty() {
            return Ok(Response::new(quilt::ConfigureNodeSubnetResponse {
                success: false,
                error: "subnet is required".to_string(),
            }));
        }

        let current_subnet = self
            .sync_engine
            .get_node_subnet_cidr()
            .await
            .map_err(|e| Status::internal(format!("Failed to read node subnet: {}", e)))?;
        ...
        if self.has_active_container_state().await? {
            return Ok(Response::new(quilt::ConfigureNodeSubnetResponse {
                success: false,
                error: "cannot change node subnet while containers are running".to_string(),
            }));
        }
        ...
        let active_allocations = self
            ...
            .await
            .map_err(|e| Status::internal(format!("Failed to list network allocations: {}", e)))?
            .into_iter()
            .any(|a| a.status != crate::sync::network::NetworkStatus::Cleaned);
        if active_allocations {
            return Ok(Response::new(quilt::ConfigureNodeSubnetResponse {
                success: false,
                error:
                    "cannot change node subnet with active network allocations; cleanup required"
                        .to_string(),
            }));
        }

        match self.sync_engine.configure_node_subnet_cidr(&subnet).await {
            Ok(()) => Ok(Response::new(quilt::ConfigureNodeSubnetResponse {
                success: true,
                error: String::new(),
            })),
            Err(e) => Ok(Response::new(quilt::ConfigureNodeSubnetResponse {
                success: false,
                error: e.to_string(),
            })),
        }
    ...
    async fn inject_route(
        &self,
        request: Request<quilt::InjectRouteRequest>,
    ) -> Result<Response<quilt::InjectRouteResponse>, Status> {
        let req = request.into_inner();
        match self
            .network_manager
            .inject_route(&req.destination, &req.via_interface)
            .await
        {
            Ok(()) => Ok(Response::new(quilt::InjectRouteResponse {
                success: true,
                error: String::new(),
            })),
            Err(e) => Ok(Response::new(quilt::InjectRouteResponse {
                success: false,
                error: e,
            })),
        }
    ...
    async fn remove_route(
        &self,
        request: Request<quilt::RemoveRouteRequest>,
    ) -> Result<Response<quilt::RemoveRouteResponse>, Status> {
        let req = request.into_inner();
        match self.network_manager.remove_route(&req.destination).await {
            Ok(()) => Ok(Response::new(quilt::RemoveRouteResponse {
                success: true,
                error: String::new(),
            })),
            Err(e) => Ok(Response::new(quilt::RemoveRouteResponse {
                success: false,
                error: e,
            })),
        }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    ConsoleLogger::success(
        "🚀 Quilt server running with SQLite sync engine - non-blocking operations enabled",
    );
    ...
    tokio::select! {
        result = Server::builder()
            ...
            .add_service(QuiltRuntimeServer::new(service.clone()))
            .serve(addr) => {
            result?;
        }
