...
pub fn create_pty_with_size(rows: u16, cols: u16) -> Result<PtyPair, String> {
    ...
    let pty_result =
        openpty(Some(&winsize), None).map_err(|e| format!("Failed to create PTY: {}", e))?;
...
pub fn resize_pty(master: RawFd, rows: u16, cols: u16) -> Result<(), String> {
    ...
    let result = unsafe { ioctl(master, TIOCSWINSZ, &winsize as *const Winsize) };
...
pub fn setup_slave_pty(slave: RawFd) -> Result<(), String> {
    ...
    let tty = File::open("/dev/tty").map_err(|e| format!("Failed to open /dev/tty: {}", e))?;
...
pub fn make_pty_nonblocking(fd: RawFd) -> Result<(), String> {
    ...
    // Get current flags
    let flags =
        fcntl(fd, FcntlArg::F_GETFL).map_err(|e| format!("Failed to get fd flags: {}", e))?;
    ...
    // Set new flags
    fcntl(fd, FcntlArg::F_SETFL(flags)).map_err(|e| format!("Failed to set fd flags: {}", e))?;
