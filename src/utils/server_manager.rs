...
pub async fn check_server_health() -> bool {
    ...
    matches!(
        tokio::time::timeout(Duration::from_secs(2), channel_result.unwrap().connect()).await,
        Ok(Ok(_))
    )
}
...
pub fn stop_daemon(force: bool) -> Result<(), String> {
    let pid = read_pid_file().ok_or("Daemon is not running (no PID file)")?;
...
mod tests {
    ...
    #[test]
    fn test_write_read_pid_logic() {
        // Test PID file write/read logic using a temporary directory
        // (since /var/run/quilt requires root permissions)
        let test_dir = env::temp_dir().join("quilt-test-pids");
        std::fs::create_dir_all(&test_dir).unwrap();

        let test_pid_file = test_dir.join("quilt.pid");
        let test_pid = 12345u32;

        // Write PID
        std::fs::write(&test_pid_file, test_pid.to_string()).unwrap();
